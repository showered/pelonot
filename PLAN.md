...
**Fifty-two entries have passed through it.** In order: standing and seated
...
now the thing in the way), and **the code shown by default** (**15.6.15**).
...
### Latest session — 5 August 2026 (thirty-fifth sitting): the three dots, and a lock nobody could see
...
### The sitting before — 5 August 2026 (thirty-fourth sitting): the whole first-run journey, and the code that outlived the screen showing it
...
**The owner's inbox is empty. The deadlock that stopped the QR journey is
fixed and reproduced-then-unreproduced; what stands between that fix and the
owner is one deploy, and what stands between the *journey* and anybody is the
mailer.**

1. **Redeploy the web app — `link.js` carries 15.6.14's fix and reaches nobody
   until it is pushed.** The owner redeployed at the start of this sitting and
   `./web/check-deployed.sh` reported seven files the same; this sitting has
   drifted it again on purpose, and the fix in it is the one they reported.
   **17.16.9 — the pairing page's voice — is still the work to do in the same
   push**, and it is now overdue rather than merely queued: two fixes have
   waited on a deploy command that **17.16.2** says is written down nowhere.
...
6. **22.7.4 — the date header that did not move when the row did.** Unchanged
   for three sittings, and it **needs a decision rather than a fix**.
