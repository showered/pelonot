...
### Latest session — 13 August 2026 (forty-seventh sitting): the app has drawn heart-rate zones for three sittings and never once said how long a ride spent in them
...
### The sitting before — 13 August 2026 (forty-sixth sitting): the app kept promising to reconnect to a port that was never coming back
...
thirteenth sitting running, because none of it is work: what stands between the
...
reason. What moved this sitting is again not on this list: **21.4.1**, time in
each heart-rate zone, in a phase this project's own summary page calls done.

**A fourth way of reading this list, and it is the one to try next time.** The
last three sittings mined *written-down claims nobody re-checks* and then the
*severity-ranked list of what is wrong*; both are worked out — `STATUS.md` has
nothing left in it that is ours and unblocked. So this sitting **counted the
unticked boxes per phase file** and looked at the outlier: **Phase 21 is 20
unticked of 31**, against Phase 22's 4 of 50 and Phase 24's 3 of 45, in a phase
`STATUS.md` calls done. That is a two-minute `grep -c` and it found both a box
that had been built for three sittings and never crossed off (21.2.3) and a
feature that was genuinely missing with nothing on any screen looking wrong
(21.4.1). **The ratio is the signal, not the count** — a phase with a long tail
of deliberately-deferred ideas looks the same from a distance, which is why the
next step is reading the boxes rather than trusting the number.

**And the sitting's most useful finding was again in what the change *touched*
rather than in what it added.** `RetentionRepository` had built its charts
without a maximum heart rate for as long as the trimmer has existed, correctly —
and 21.4.1 turned that number into a denominator, which would have made the next
trim freeze an empty distribution onto the row and lose the answer for good.
