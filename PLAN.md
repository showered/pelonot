...
### Latest session — 16 August 2026 (fifty-sixth sitting): three defects on the path a rider walks once, and every one of them invisible to anybody who had walked it before
...
### The sitting before — 15 August 2026 (fifty-fifth sitting): a number for a rider that only goes up, and two faults a diff could not have shown
...
**The owner's inbox is empty and stayed empty** — nothing came in, so the sitting
picked its own work. The top of this list is unchanged for the twenty-second
sitting running, because none of it is work: what stands between the QR fix and
the owner is one deploy, and what stands between the *journey* and anybody is
the mailer. 15.3.2 is built and unticked and item 0 is still its reason.
...
**The seam this sitting found is that a trigger which cannot change value cannot
fire twice.** Two of the three defects are the same sentence: an effect keyed on
something that was already true, waiting on something that was not yet. The
account offer asked for a pairing code before the profile existed and never
asked again; the notification permission was composed below the gate that
decides when it may be asked, so the first moment it could fire was already
mid-ride. **The rule it leaves behind: whatever a one-shot needs, put it in the
key** — a precondition checked inside the body and absent from the trigger is a
precondition nobody will notice failing, which is the fifty-fourth sitting's
`maxHr` default in a new costume.

**And the reason all three survived this long is the audience.** Every one is
first-run only, on a path the person doing the checking has walked a hundred
times with a profile already made and the permissions already granted. 20.4.3
said the path and not the screen is the unit; what the walk adds is that
**`pm clear` is the instrument**, and that the guest layout (12.7) and the
never-ridden profile are the two states a developer never sees by accident.

**Phase 20 moved from 19 of 31 to 23 of 35** — 20.4.3 ticked at last, with
20.4.7, 20.4.8 and 20.4.9 built and observed, and **20.4.10** written down
rather than built. Everything else is unchanged: **Phase 15 (22 of 64)** is
still the outlier, followed by **Phase 8 (16 of 53)**, **Phase 11 (15 of 70)**,
**Phase 21 (15 of 32)** and **Phase 17/18 (14 of 44)**. Phases 27 and 28 are
18/18 and 24/24 and both are deliberately untouched — the owner's own weighting.

**Two notes about the test device**, because both change what a measurement on
it means. The 49 hand-seeded rides (`lvl-simon-*`, `lvl-alex-*`) that gave the
level badge something to read **are gone** — this sitting cleared the app four
times to walk the first-run path, and they were `workouts` rows with no
`workout_metrics` behind them anyway, so anything counting samples or sizing
storage (23.4.1) wanted a clean profile regardless. And **the AVD's Gboard can
