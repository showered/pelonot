...
### Latest session — 14 August 2026 (fifty-first sitting): the app asked a question no normal person can answer, and then averaged in the seconds nobody rode
...
### The sitting before — 14 August 2026 (fiftieth sitting): the ride was not hard to re-file, it was invisible
...
seventeenth sitting running, because none of it is work: what stands between the
...
reason. What moved this sitting is again not on this list: **21.1.6** and
**19.1.2b**, both of which had the owner's own words attached and needed nothing
a session did not already have.

**The seam this sitting mined is a fourth member of the family, and it is the
cheapest one yet: an item that had already been answered somewhere else in the
app.** The last four sittings looked for *claims that had gone stale*, for
*blockers that had quietly been cleared*, and for *items describing a smaller
problem than the code has*. 19.1.2b is none of those — it asked an open
question, *over the whole recording or over the pedalling?*, and
`AutoPausePolicy` had been answering it in code since 19.1.2, stopping the clock
on exactly that judgement. **The tell is an item that poses a decision the app
visibly makes somewhere else**, and the fix is usually to make the two agree
rather than to decide anything.

**21.1.6 is the other kind and is worth naming beside it: an item built on one
of its two screens and never checked on the other.** Onboarding had come out
right by a different route (21.1.1b), which is exactly what made Settings easy
to believe was done. **Two screens asking one question is where an item quietly
half-lands.**

**The box-counting lead is still live and Phase 19 has joined the top of it.**
After this sitting: **Phase 17/18 (26 of 44)** is still the outlier but is almost
all web-app and cloud work, which is owner-and-deploy rather than a session's;
**Phase 15 (21 of 63)**, **Phase 8 (16 of 53)**, **Phase 21 (15 of 32)**,
**Phase 11 (15 of 70)** and **Phase 19 (14 of 23)** are the ones worth reading
box by box. The rule holds — the ratio is a signal and the boxes are the answer.

**Four things this sitting deliberately wrote down rather than built**, all four
found by reading code under a one-line item:

- **19.1.2c** — a stop filed as *Active Recovery*, which is 19.1.2b's decision
  reaching one card further. Left because it is a **count** of seconds, and 23.4
  has already settled that a count cannot be recomputed after a trim; the fix is
  at the moment of counting and 21.4.1 has already solved this exact problem for
  heart rate.
- **15.3.7** — a restored profile comes back without its heart-rate zones.
  Blocked on a cloud column, and on 15.4.3's precedent an app half written
  against a schema nobody has deployed fails in the way that reads like a broken
  feature.
- **11.2.1a** — annotated rather than picked up: 11.7.3 stopped drawing the
  resistance band anywhere, so its symptom cannot be seen and it is a modelling
  observation now.
- **11.1b.8** — the overlay eating touches, which has a better fix shape than its
  own text names but wants the owner's eye and cannot be judged on an AVD at all
  (`FLAG_SECURE` returns a black screenshot over playing video).
