...
### Latest session — 16 August 2026 (fifty-seventh sitting): three notes in the inbox, and the screen the owner keeps coming back to now fills itself
...
### The sitting before — 16 August 2026 (fifty-sixth sitting): three defects on the path a rider walks once, and every one of them invisible to anybody who had walked it before
...
**The owner's inbox had three entries and is empty again** — 22.9, Phase 29 and
21.7 are where they went, and the plan's own rule is that they now take their
place in this list rather than the top of it. The top is unchanged for the
twenty-third sitting running, because none of it is work: what stands between the
QR fix and the owner is one deploy, and what stands between the *journey* and
anybody is the mailer. 15.3.2 is built and unticked and item 0 is still its
reason.

**Two of the new entries need the owner or the bike and are named here rather
than buried in their phases.** **29.1.1 is three adb commands on the tablet** —
does Android 11 have Health Connect, is there a Play Store to install it from —
and the whole of Phase 29 is downstream of the answer. **29.1.2 is the owner's
own decision**: `connect-client` declares minSdk 26 and this app is 24, and
raising the floor costs nothing measurable but is a stated compatibility claim.
**21.7.1 needs the friend with the Apple Watch** and nothing else: install a
broadcaster, open Settings, see whether a strap appears. And **29.2.2 needs an
...
**The redeploy is still the one thing worth asking for, and it has a command
beside it.** `./web/check-deployed.sh` reports six files drifted, and the app on
the internet is the two-view one. `cd web && npx wrangler deploy` is the
reconstruction (17.16.2); the check is what says whether it worked, and it also
reports that the deployed `config.js` is still on the legacy key form (17.16.3).

**The seam this sitting found is that two notes about one screen can pull in
opposite directions, and the second one's obvious fix is the first one's
defect.** 22.8 said the dashboard was *stretched* and the answer took it from
993 dp to 513; 22.9 says the room that bought should not be left empty. The
obvious way to fill it — weight the rows and let the cards grow — is 22.8's
complaint restored exactly, and the honest way is *more things the rider wants,
not more of the same thing*. **Both traps were still walked into**, and both were
caught by looking at the tablet: two doors stacked in their old shape demanded a
296 dp row, and the day grid drawn beside the household as well as instead of it
took the screen back to 990 dp. **The residue is a question to ask of any
density change: does it add content, or does it add air.**

**And the previous sitting's rule about audiences held again.** The state that
was 42% empty is a bike with **one rider on it**, because the rail is the
household panel and it draws nothing below two — the ordinary case for a new
bike, and the one the 22.8 measurements were not taken in. It is the same shape
...
**The test device now has 45 seeded rides on it again, and a second profile.**
`seed-robin-*` (38 rides across 15 weeks) and `seed-alex-*` (7 in the last month,
profile 2) were inserted by hand so the day grid, the household panel and a
suggestion that is not a recovery class could be looked at at all — the tablet's
one real ride drew seventeen weeks of grey. **They are `workouts` rows with no
`workout_metrics` behind them**, exactly like the `lvl-*` fixtures before them,
so anything counting samples or sizing storage (23.4.1) wants them cleared first,
and `power_provenance` on every one of them is `Modelled`, so nothing gated on
measured power will look at them. And **the AVD's Gboard can
sit in a floating mode that reports zero IME insets**, which hides exactly the
class of defect `hw.keyboard=no` was set to expose (20.4.2): the docked keyboard
came back after `pm clear com.google.android.inputmethod.latin`, and a screen
checked under the floating one has not been checked.
