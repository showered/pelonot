...
### Latest session — 14 August 2026 (forty-eighth sitting): an item that said what was blocking it, in a plan where the blocker had quietly been built
...
### The sitting before — 13 August 2026 (forty-seventh sitting): the app has drawn heart-rate zones for three sittings and never once said how long a ride spent in them
...
fourteenth sitting running, because none of it is work: what stands between the
...
reason. What moved this sitting is again not on this list: **21.6.3**, a ride
held against the class it was ridden to, in a phase this project's own summary
page calls done.

**The reading technique that found it, stated so the next sitting can use it
again.** The previous sitting counted the unticked boxes per phase file and
went at the outlier — Phase 21, still 18 of 31 — and its own advice was that
**the ratio is a signal and the boxes are the answer**, so this sitting read
them. What it found was better than a missing feature: **an item that names its
own blocker, in a plan where the blocker had since been built.** 21.6.3 says
plainly *"nothing yet draws an HR zone for a past ride"*, and 21.2.3 and 21.4.1
had made that false two sittings earlier. That is the same seam as the last four
sittings — *written-down claims nobody re-reads* — read from the other end, and
it is the cheapest version of it: an unticked box that explains what it is
waiting for is a claim with a date on it. **Phase 21 is 18 unticked of 32 now,
which is exactly where it started** — one box ticked and one opened (21.4.2c),
which is what building an item honestly usually does to a count and is the
reason the ratio is a lead rather than a score. Phase 15 (20 of 62) and Phase 8
(16 of 53) are the next two worth the same treatment.

**And the sitting's two best findings were again in the *looking* rather than in
the building.** The zone list did not end in a full stop and `formatDuration`
said *"1 minutes 42 seconds"* — both four sittings old, both on a card anybody
could have read, and neither visible in any diff. Every UI item in this plan is
owed one screenshot that somebody actually reads.
