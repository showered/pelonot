...
| 18 | Social **across bikes** — the networked tier | 🔶 **The class leaderboard is built (18.5), and the dashboard's shared activity and kudos are implemented but awaiting a signed-in device check (18.2–18.3).** The feed now reacts when an account is attached to the selected profile, distinguishes no shared rides from an unreachable service, keeps Room activity during a cloud failure, and lets the rider give or remove kudos through the existing visibility-checked RPC. A rider who is also on this tablet appears from Room once. The owner chose no friend graph for the small group (18.11), so friends and requests (18.1) are deferred. The class comparison (18.4), cross-bike live ghost (18.12), and class leaderboard failure state (18.10) remain open; Phase 24 is still the offline household foundation |
