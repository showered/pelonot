...
| # | What | Why it blocks |
|---|------|--------------|
| ~~**19.1.4**~~ | ~~CI is written and never green~~ | **Closed, 17 August 2026.** The fifty-eighth sitting found why it had never run — `ci.yml` triggers on push to `main` and every commit was on `setup` — and the owner cleared it by merging and pushing. Green on the first attempt. **The gap that remains is the instrumented suite**, which CI does not run because the suite is order-dependent (item 6, below): the database, the service and the migrations are tested only on somebody's machine |
...
sentence about the overlay, and *Set up · Four questions, then ride*. **And CI is green (19.1.4)**, so the project can take a patch: the run the
workflow had been waiting eighteen sittings for fired the moment the owner
merged to `main`, and passed first time. What is left on that list is the
instrumented suite, which CI still does not run (item 6). That is a
