...
**Latest verification: 17 September 2026.** The dashboard now leads with
Choose a class. FTP distinguishes a starting value from ride-supported evidence,
shows changes and offers explicit reviews. The missed effort-answer reassessment
is fixed. **983 JVM tests and 132 instrumented tests**, debug/release builds and
lint pass; see [REVIEW.md](REVIEW.md) for emulator evidence.
No production release was published. The older feature summary below is
historical; [PLAN.md](PLAN.md) is the current index.
...
| Measured | |
|---|---|
| JVM tests | **983**, 0 failures |
| Plan boxes ticked | **748 of 968** — 77% |
...
**Done as the plan is written — the percentage is at the top of this page, and
it will never be 100.** *(It used to be restated here, from a different count,
and the two disagreed: 651 of 863 above against 577 of 801 here. That is
19.1.7a's whole case in one page, and the fix is that neither number is typed
any more.)* The boxes left are not a queue. They are a place ideas are kept
