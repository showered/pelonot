...
**Written 4 August 2026, updated 12 August (fortieth sitting).**
Measured, not estimated: `assembleDebug` passes, **711 JVM tests and 83
instrumented tests, 0 failures**, and **554 of 791 plan boxes** are ticked
...
| **2. The record** | History, charts, FTP, heart-rate zones, export, migrations | ✅ **Done**, bar the cosmetic backlog. Retention (23.4) has **nothing in software in front of it any more**: a ride's mean-maximal efforts (16.3.3a) and where its watts came from (23.4.12) are both written down when the ride is recorded, so trimming its samples leaves the rider's bests, their place on every leaderboard and the dashboard's verdict exactly where they were. Both were measured against the previous build on the same trimmed data, which loses all of it and says nothing. What is left is one measurement on the bike (23.4.1) and the trimmer itself. **And restore has been fixed after refusing every backup this build made** — a version number kept equal to another version number by a comment, which drifted (19.1.3a) |
...
- **23.4 retention** — condensing old rides to aggregates. **Both of its
  software prerequisites are now built**: a ride's mean-maximal efforts
  (16.3.3a) and where its watts came from (23.4.12) are worked out once when the
  ride is recorded and kept on the row, so trimming its samples leaves the
  rider's bests, their place on every board and the dashboard's *best you've
  ridden it* verdict exactly where they were. Both were measured against the
  previous build on the same trimmed data: it loses all four bests and blames the
  wrong ride, and it drops the whole *On this bike* card off the class screen
  without saying so. What is left in front of trimming is **23.4.1**, one
  measurement on the bike that everything below it is sized off, and then the
  trimmer — with three rules the audits leave it (exclude rides in progress,
  enforce 23.4.6 in the trimmer not the sync worker, and 23.4.10's two meanings
  of one dialog).
...
**Done as the plan is written: 70%, and it will never be 100.** 554 of 791
...
boxes between them, most of Phase 27's, and an audit two sittings ago closed one box
by opening another (23.4.9 → 23.4.12), which this sitting then closed. A closed box and an open one are not the same
