...
**Latest verification: 22 September 2026.** Update installation now rechecks for
an active ride after copying the APK into Android's installer, closing a gap
where a ride could start during that copy. Preparation cancellation and retry
are regression-tested. **987 JVM tests and the debug build** pass. Actual
package replacement with the permanent signing key remains to be rehearsed.
The earlier live-board emulator check is recorded in [PLAN.md](PLAN.md).
