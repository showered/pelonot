...
| **17.16.2** | ~~How the web app is deployed~~ **— closed. What blocks now is that a deploy has not happened.** | The command is in the repo at last (`web/wrangler.jsonc`, `cd web && npx wrangler deploy`), labelled a reconstruction rather than a transcript because it has not been run from here. `./web/check-deployed.sh` reports **six files drifted**: the live site is the two-view web app, and everything built in the fifty-third sitting reaches nobody until somebody pushes it |
