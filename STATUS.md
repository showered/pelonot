...
**Written 4 August 2026. The prose carries a date; the numbers below do not
need one** — they are emitted by `tools/status-figures.sh` and CI fails on a
figure that has drifted (19.1.7a). `assembleDebug` passes, and the **whole
130-test instrumented suite was green in the sixty-fourth sitting**, migrations
included; that one is a claim about a machine nobody can re-run in CI, so it
keeps its sitting.
...
| Plan boxes ticked | **705 of 939** — 75% |
